...
class MolFeatureExtractor:
    ...
    def _batch_encode(self, smiles: list[str]) -> np.ndarray:
        ...
        # Wrap _encode as a ray remote function
        encode_remote = ray.remote(MolFeatureExtractor._encode)
        ...
        for smis_batch in tqdm(self._batches(smiles, batch_size), total=n_batches):
            refs = [
                encode_remote.remote(mols_chunk)
    ...
    def standardise_features(self, features: np.ndarray) -> np.ndarray:
        if not hasattr(self, '_scaler'):
            if os.path.exists(self.scaler_path / f"std.bin"):
                self._scaler = joblib.load(self.scaler_path / f"std.bin")
            else:
                self._scaler = StandardScaler()
                self._scaler.fit(features)
                joblib.dump(self._scaler, self.scaler_path / f"std.bin")
