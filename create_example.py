...
# ChEMBL IDs (First 10 targets)
query_chembl = "SELECT chembl_id FROM target_dictionary WHERE target_type = 'SINGLE PROTEIN' AND organism = 'Homo sapiens' LIMIT 10 OFFSET 0"
chembl_ids = pd.read_sql_query(query_chembl, conn)['chembl_id'].tolist()

# Target Names (Next 10 targets)
query_name = "SELECT pref_name FROM target_dictionary WHERE pref_name IS NOT NULL AND target_type = 'SINGLE PROTEIN' AND organism = 'Homo sapiens' LIMIT 10 OFFSET 10"
names = pd.read_sql_query(query_name, conn)['pref_name'].tolist()

# UniProt Accessions (Next 10 targets)
query_acc = """SELECT cs.accession FROM target_dictionary td 
JOIN target_components tc ON td.tid = tc.tid 
JOIN component_sequences cs ON tc.component_id = cs.component_id
WHERE cs.accession IS NOT NULL AND td.target_type = 'SINGLE PROTEIN' AND td.organism = 'Homo sapiens' LIMIT 10 OFFSET 20"""
accessions = pd.read_sql_query(query_acc, conn)['accession'].tolist()

# Gene Symbols (Next 10 targets)
...
WHERE csy.syn_type = 'GENE_SYMBOL' AND td.target_type = 'SINGLE PROTEIN' AND td.organism = 'Homo sapiens' LIMIT 10 OFFSET 30"""
